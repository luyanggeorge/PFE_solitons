...
if nic=="SP1": # soliton decaying sufficiently fast to be taken as periodi using BL expansion in z
    ...
    nCGvert = 4 # function space order vertical
    nvpcase = "MMP" # MMP=modified midpoint VP time discretisation in case of making more cases; SV=Stormer-Verlet
    nphihatz = "Unity" # "unity": phihat=1.0; 1: "GLL1" 1st GLL at nCGvert, etc.
    ...
    cxx=3*5*4/nCG
    # else: c=12
    nx = int(np.round(cxx*(x2hat-x1hat)))
    ny = int(np.round(cxx*Lyhat))
    nz = 4
...
if nic=="linearw": # linear waves in x-direction dimensional
    ...
    ttilde = 23.75 # BLE final time units used for SP3 in BLE
...
if nsavelocalHPC==1: # save_path =  "/nobackup/$USER/lin_pot_flow/"
    save_path =  "/nobackup/mmyl/SP3_OJ/CG2-dy3-dt2/"
else:
...
while t <= 1.0*(t_end +dt): #  t_end + dt
    ...
    # err.interpolate(varphimp+psi_f)
     # total velocity potential for plotting
    # EKin0.interpolate( 0.5*( LwdWw2*(fac*H0+hmp)*(u0z + (psimp*phihat+varphimp).dx(0)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz +(psimp*phihat+varphimp).dx(2)))**2 \
    #                           + (fac*H0+hmp)*(u0zpy + (psimp*phihat+varphimp).dx(1)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + (psimp*phihat+varphimp).dx(2)))**2 \
    #                           + (H0**2/(fac*H0+hmp))*(u0zpz +(psimp*phihat+varphimp).dx(2))**2 )  )
    # EPot0.interpolate( gg*H0*(fac+hmp/H0)*(x[2]*(fac*H0+hmp)/H0-H0) )
    
    # Energy monitoring: bit too frequent reduce
    
    # EKin = fd.assemble( 0.5*( LwdWw2*(fac*H0+h_old)*(u0z + (varphi.dx(0))-(1.0/(fac*H0+h_old))*(H0*btopo.dx(0)+x[2]*h_old.dx(0))*(u0zpz + varphi.dx(2)))**2 \
    #                           + (fac*H0+h_old)*(u0zpy + (varphi.dx(1))-(1.0/(fac*H0+h_old))*(H0*btopo.dx(1)+x[2]*h_old.dx(1))*(u0zpz + varphi.dx(2)))**2 \
    #                           + (H0**2/(fac*H0+h_old))*(u0zpz +varphi.dx(2))**2 ) * fd.dx(degree=vpoly) )
    # EKin = fd.assemble(  EKin0* fd.dx(degree=vpoly) )
    # EKin = fd.assemble( 0.5*( LwdWw2*(fac*H0+hmp)*(u0z + varphi.dx(0)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz + varphi.dx(2)))**2 \
    #                           + (fac*H0+hmp)*(u0zpy + varphi.dx(1)-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + varphi.dx(2)))**2 \
    #                           + (H0**2/(fac*H0+hmp))*(u0zpz +varphi.dx(2))**2 ) * fd.dx(degree=vpoly) )
    # EKin = fd.assemble( 0.5*( LwdWw2*(fac*H0+hmp)*(u0z + varphi.dx(0)*phihat-(1.0/(fac*H0+hmp))*(H0*btopo.dx(0)+x[2]*hmp.dx(0))*(u0zpz + varphi.dx(2)))**2 \
    #                           + (fac*H0+hmp)*(u0zpy + varphi.dx(1)*phihat-(1.0/(fac*H0+hmp))*(H0*btopo.dx(1)+x[2]*hmp.dx(1))*(u0zpz + varphi.dx(2)))**2 \
    #                           + (H0**2/(fac*H0+hmp))*(u0zpz +varphi.dx(2))**2 ) * fd.dx(degree=vpoly) )
                    
           
    # EPot = fd.assemble( EPot0*fd.dx(degree=vpoly) )
    # EPot = fd.assemble( gg*H0*(0.5*(H0+hmp)**2-H0*(H0+hmp)) * fd.dx(degree=vpoly) )
    
    # EKin0.interpolate( 0.5*( LwdWw2*(fac*H0+h_old)*(u0z + (psimp*phihat+varphimp).dx(0)-(1.0/(fac*H0+h_old))*(x[2]*h_old.dx(0))*(u0zpz +(psimp*phihat+varphimp).dx(2)))**2 \
    #                           + (fac*H0+h_old)*(u0zpy + (psimp*phihat+varphimp).dx(1)-(1.0/(fac*H0+h_old))*(x[2]*h_old.dx(1))*(u0zpz + (psimp*phihat+varphimp).dx(2)))**2 \
    #                           + (H0**2/(fac*H0+h_old))*(u0zpz +(psimp*phihat+varphimp).dx(2))**2 )  )
    # EPot0.interpolate( gg*H0*(fac+h_old/H0)*(x[2]*(fac*H0+h_old)/H0-H0) )
    ...
    # 'old'
    # EKin=fd.assemble( 0.5*( LwdWw2*(fac*H0+h_old)*(u0z + err.dx(0)-(1.0/(fac*H0+h_old))*(x[2]*h_old.dx(0))*(u0zpz +(psi_f+varphimp).dx(2)))**2 \
    #                           + (fac*H0+h_old)*(u0zpy + (psi_f+varphimp).dx(1)-(1.0/(fac*H0+h_old))*(x[2]*h_old.dx(1))*(u0zpz + (psi_f+varphimp).dx(2)))**2 \
    #                           + (H0**2/(fac*H0+h_old))*(u0zpz +(psi_f+varphimp).dx(2))**2 )* fd.dx(degree=vpoly)  )
    # EPot=fd.assemble( gg*H0*(fac+h_old/H0)*(x[2]*(fac*H0+h_old)/H0-H0)* fd.dx(degree=vpoly) )
    
    # 'mp'
    # EKin=fd.assemble( conditional(x[1]<45000,0.5*( LwdWw2*(fac*H0+hmp)*(u0z + (psimp*phihat+varphimp).dx(0)-(1.0/(fac*H0+hmp))*(x[2]*hmp.dx(0))*(u0zpz +(psimp*phihat+varphimp).dx(2)))**2 \
    #                           + (fac*H0+hmp)*(u0zpy + (psimp*phihat+varphimp).dx(1)-(1.0/(fac*H0+hmp))*(x[2]*hmp.dx(1))*(u0zpz + (psimp*phihat+varphimp).dx(2)))**2 \
    #                           + (H0**2/(fac*H0+hmp))*(u0zpz +(psimp*phihat+varphimp).dx(2))**2 ),0.0 )* fd.dx(degree=vpoly) )
    # EPot=fd.assemble(conditional(x[1]<45000,gg*H0*(fac+hmp/H0)*(x[2]*(fac*H0+hmp)/H0-H0),0.0)* fd.dx(degree=vpoly) )
