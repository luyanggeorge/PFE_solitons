# ignore O(mu^2) in the phi expression
...
def test_case():
    ...
    dim = '3D'    # "2D": SP1; "3D": SP2 or SP3
    save_path = 'data/SP2_test_SV'
    ...
    return scheme, dim, save_path, save_pvd, soliton, nCG
...
def IC_3D(soliton, g, H0, epsilon, mu, tan0):
    ...
    if soliton=='SP2': # k2=k3=0, k1=-k4
        ...
        def h_ex_expr(x,y):  # h(x,y,0) = H0 + eta(x,y,0)
            function=( H0 + 4*H0*c2*epsilon*k4**2*(exp(k4*(c1**2*epsilon*k4*y/H0 + 2*c1*sqrt(epsilon)*x/H0)) + 2*exp(c1*sqrt(epsilon)*k4*x/H0) + exp(c1**2*epsilon*k4**2*y/H0))\
                       *exp(c1*sqrt(epsilon)*k4*x/H0)/(4*exp(2*k4*(c1**2*epsilon*k4*y/H0 + c1*sqrt(epsilon)*x/H0)) + 4*exp(k4*(c1**2*epsilon*k4*y/H0 + c1*sqrt(epsilon)*x/H0))\
                        + 4*exp(k4*(c1**2*epsilon*k4*y/H0 + 3*c1*sqrt(epsilon)*x/H0)) + exp(4*c1*sqrt(epsilon)*k4*x/H0) + 2*exp(2*c1*sqrt(epsilon)*k4*x/H0) + 1) )
            return function
        ...
        def Phi_2ed_expr(x,y): # Phi_xx(x,y,0)+Phi_yy(x,y,0)
            function = 4*c1**2*c3*epsilon**(3/2)*k4**3*(-c1**2*epsilon*k4**2*(2*exp(c1*k4*(c1*epsilon*k4*y + sqrt(epsilon)*x)/H0) - 2*exp(c1*k4*(c1*epsilon*k4*y + 3*sqrt(epsilon)*x)/H0)\
                       + exp(4*c1*sqrt(epsilon)*k4*x/H0) - 1)*exp(c1*k4*(c1*epsilon*k4*y + sqrt(epsilon)*x)/H0) + (-exp(c1*k4*(c1*epsilon*k4*y + 4*sqrt(epsilon)*x)/H0)\
                       - 2*exp(c1*k4*(2*c1*epsilon*k4*y + sqrt(epsilon)*x)/H0) + 2*exp(c1*k4*(2*c1*epsilon*k4*y + 3*sqrt(epsilon)*x)/H0) - 4*exp(3*c1*sqrt(epsilon)*k4*x/H0)\
                       + 4*exp(c1*sqrt(epsilon)*k4*x/H0) + exp(c1**2*epsilon*k4**2*y/H0))*exp(c1*sqrt(epsilon)*k4*x/H0))/(mu*(8*exp(3*c1*k4*(c1*epsilon*k4*y + sqrt(epsilon)*x)/H0)\
                       + 12*exp(2*c1*k4*(c1*epsilon*k4*y + sqrt(epsilon)*x)/H0) + 6*exp(c1*k4*(c1*epsilon*k4*y + sqrt(epsilon)*x)/H0) + 12*exp(2*c1*k4*(c1*epsilon*k4*y + 2*sqrt(epsilon)*x)/H0)\
                       + 12*exp(c1*k4*(c1*epsilon*k4*y + 3*sqrt(epsilon)*x)/H0) + 6*exp(c1*k4*(c1*epsilon*k4*y + 5*sqrt(epsilon)*x)/H0) + exp(6*c1*sqrt(epsilon)*k4*x/H0)\
                       + 3*exp(4*c1*sqrt(epsilon)*k4*x/H0) + 3*exp(2*c1*sqrt(epsilon)*k4*x/H0) + 1))
            return function

        def phi_ex_expr(Phi, Phi_2, z): # phi(x,y,z,0), here z is z_bt, ignore the O(mu^2) term
            function=( a_phi*(Phi - 0.5*mu*pow(z/H0,2)*Phi_2) )
            return function
        ...
        def tilde_phi_expr(function, Phi, Phi_2, phi_corr, z):
            function.interpolate( a_phi*(Phi - 0.5*mu*pow(z/H0,2)*Phi_2) - phi_corr )

    return h_ex_expr, Phi_expr, Phi_2ed_expr, phi_ex_expr, phi_corr, tilde_phi_expr
