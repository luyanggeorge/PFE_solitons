...
# C, B, FWF, b, H removed.
# U0, c0 evaluated once at the beginning
...
scheme, dim, save_path, save_pvd, soliton, nCG = test_case()
...
if soliton=='SP1':
    ...
    h_ex_expr, Phi_expr, Phi_2ed_expr, phi_ex_expr, phi_corr, tilde_phi_expr \
...
if scheme=="SE": #_________ Symplectic-Euler scheme _________#
    ...
    h_n0 = Function(V, name='total_depth')               # h^n
    psi_1_n0 = Function(V, name='tilde_phi_f') # tilde_psi_1^n
...
if soliton == 'SP1':
    ...
    U_0 = (phi_ex_expr(Phi_1_x2,Phi_2_x2,h_x2)-phi_ex_expr(Phi_1_x1,Phi_2_x1,h_x1))/Lx
    c_0 = 0.5*(phi_ex_expr(Phi_1_x1,Phi_2_x1,h_x1)+phi_ex_expr(Phi_1_x2,Phi_2_x2,h_x2))
    ...
    tilde_phi_expr(psi_1_n0, Phi_1, Phi_2, phi_corr_exp, h_n0)
    ...
    for i in range(0,n_z):
        ...
        U_0_i = (phi_ex_expr(Phi_1_x2,Phi_2_x2,z_x2)-phi_ex_expr(Phi_1_x1,Phi_2_x1,z_x1))/Lx
        c_0_i = 0.5*(phi_ex_expr(Phi_1_x1,Phi_2_x1,z_x1)+phi_ex_expr(Phi_1_x2,Phi_2_x2,z_x2))
        phi_corr_exp = phi_corr(x[0], U_0_i, c_0_i) # correction function at z*_i, UFL expression
        
        tilde_phi_expr(phii_z, Phi_1, Phi_2, phi_corr_exp, z_bt)
...
if scheme=="SE": #_____________________________________________ Symplectic-Euler ______________________________________________#
    ...
    WF_h_psi = SP_solvers.WF_h_SE(dim, n_z, g, H0, dt, delta_psi, delta_hat_star, h_n0, h_n1, psi_1_n0, hat_psi_star,
                                  M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN,
                                  U0_nz, hat_U0, corr_nz, hat_corr)

    #----------------------------------------- Step 2 : Update psi_1 at time t^{n+1}: -----------------------------------------#
    A_psi_s, L_psi_s = SP_solvers.WF_psi_SE(dim, g, H0, dt, delta_h, psi_1, psi_1_n0, hat_psi_star, h_n1, 
                                            M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN,
                                            U0_nz, hat_U0, corr_nz, hat_corr)

    #----------------------------------------- Step 3 : Update psi_i at time t^{n+1}: -----------------------------------------#
    A_hat, L_hat = SP_solvers.WF_hat_psi_SE_SV(dim, H0, n_z, dt, delta_hat_psi, hat_psi, h_n0, psi_1_n0, 
    ...
    WF_psi_star = SP_solvers.WF_psi_half_SV(dim, n_z, g, H0, dt, delta_h_sv, delta_hat_psi_sv, psi_1_n0, psi_1_half, hat_psi_star, h_n0, 
                                            M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN,
                                            U0_nz, hat_U0, corr_nz, hat_corr)

    #----------------------------- Step 2 : Update h^{n+1} and psi_i at time t^** simulataneously: ----------------------------#
    WF_h_psi = SP_solvers.WF_h_SV(dim, n_z, H0, g, dt, delta_psi, delta_hat_star, h_n0, h_n1, psi_1_half, hat_psi_star, hat_psi_aux, 
                                  M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN, 
                                  U0_nz, hat_U0, corr_nz, hat_corr)

    #----------------------------------------- Step 3 : Update psi_1 at time t^{n+1}: -----------------------------------------#
    a_psi_1, L_psi_1 = SP_solvers.WF_psi_n1_SV(dim, H0, g, dt, delta_h, psi_1_half, psi_1, hat_psi_aux, h_n1, 
                                               M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN,
                                               U0_nz, hat_U0, corr_nz, hat_corr)
...
while t<=Tend+smallfac:
    ...
    if t_save-smallfac < t:
        ...
        energy = save_energy(dim, g, H0, h_n0, psi_1_n0, hat_psi_n0, U0_nz, hat_U0, corr_nz, hat_corr,
                             A11, AN1, ANN, M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN)
