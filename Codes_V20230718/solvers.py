...
def WF_h_SE(dim, n_z, g, H0, dt, b, FWF, delta_psi, delta_hat_star, h_n0, h_n1, psi_1_n0, hat_psi_star,
    ...
    if dim == "3D":
        ...
        WF_h = (H0*delta_psi*(h_n1-h_n0)/dt \
                -( h_n1*(        (psi_1_n0.dx(0)+U0_nz)*M11 +         dot((hat_psi_star.dx(0)+hat_U0),MN1))*delta_psi.dx(0)\
                  ...
                  - H0*b.dx(0)*(delta_psi.dx(0)*(B11*(psi_1_n0+corr_nz) + dot(B1N,(hat_psi_star+hat_corr))) \
                                     +delta_psi*((psi_1_n0.dx(0)+U0_nz)*B11 + dot((hat_psi_star.dx(0)+hat_U0),BN1)))\
                  - H0*b.dx(1)*(delta_psi.dx(1)*(B11*(psi_1_n0+corr_nz) + dot(B1N,(hat_psi_star+hat_corr)))\
                                     +delta_psi*((psi_1_n0.dx(1)+corr_nz.dx(1))*B11 + dot((hat_psi_star.dx(1)+hat_corr.dx(1)),BN1)))\
                  +(delta_psi/h_n1)*(  (S11*(psi_1_n0+corr_nz)+dot(SN1,(hat_psi_star+hat_corr))) * (h_n1.dx(0)**2 + h_n1.dx(1)**2)\
                               + H0*H0*(A11*(psi_1_n0+corr_nz)+dot(AN1,(hat_psi_star+hat_corr))) * (FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0)\
                              + 2.0*H0*(C11*(psi_1_n0+corr_nz)+dot(CN1,(hat_psi_star+hat_corr))) * (b.dx(0)*h_n1.dx(0) + b.dx(1)*h_n1.dx(1)) ) ))*dx 
        ...
        WF_hat_psi_star =   h_n1 * elem_mult(delta_hat_star.dx(0),(MN1*(psi_1_n0.dx(0)+U0_nz)        +dot(MNN,(hat_psi_star.dx(0)+hat_U0))))\
                          ...
                          - H0*b.dx(0)*( elem_mult(delta_hat_star,((psi_1_n0.dx(0)+U0_nz)*B1N + dot(BNN.T,(hat_psi_star.dx(0)+hat_U0))))\
                                        +elem_mult(delta_hat_star.dx(0),(BN1*(psi_1_n0+corr_nz) + dot(BNN,(hat_psi_star+hat_corr)))))\
                          - H0*b.dx(1)*( elem_mult(delta_hat_star,((psi_1_n0.dx(1)+corr_nz.dx(1))*B1N + dot(BNN.T,(hat_psi_star.dx(1)+hat_corr.dx(1)))))\
                                        +elem_mult(delta_hat_star.dx(1),(BN1*(psi_1_n0+corr_nz) + dot(BNN,(hat_psi_star+hat_corr)))))\
                 + (1.0/h_n1) * elem_mult(delta_hat_star,(SN1*(psi_1_n0+corr_nz)+dot(SNN,(hat_psi_star+hat_corr)))) * (h_n1.dx(0)**2 + h_n1.dx(1)**2)\
               + (H0*H0/h_n1) * elem_mult(delta_hat_star,(AN1*(psi_1_n0+corr_nz)+dot(ANN,(hat_psi_star+hat_corr)))) * (FWF*(b.dx(0)**2 + b.dx(1)**2)+1.0)\
              + (2.0*H0/h_n1) * elem_mult(delta_hat_star,(CN1*(psi_1_n0+corr_nz)+dot(CNN,(hat_psi_star+hat_corr)))) * (b.dx(0)*h_n1.dx(0) + b.dx(1)*h_n1.dx(1))
...
def WF_psi_SE(dim, g, H0, H, dt, b, FWF, delta_h, psi_1, psi_1_n0, hat_psi_star, h_n1, 
        ...
        L_psi_s = (H0*delta_h*psi_1_n0 \
                 -dt*( 0.5*delta_h*( ((psi_1_n0.dx(0)+U0_nz)**2)*M11 \
                    ...
                    -delta_h.dx(1)*( (psi_1_n0.dx(1)+corr_nz.dx(1))*(D11*(psi_1_n0+corr_nz) + dot(D1N,(hat_psi_star+hat_corr)))\
                       ...
                       +(H0/h_n1) * ( delta_h.dx(0)*b.dx(0) + delta_h.dx(1)*b.dx(1) - (delta_h/h_n1)*(h_n1.dx(0)*b.dx(0) + h_n1.dx(1)*b.dx(1)) )* \
                                    ( (psi_1_n0+corr_nz)**2*C11 + 2.0*dot((hat_psi_star+hat_corr),CN1)*(psi_1_n0+corr_nz) \
                                                                    + dot((hat_psi_star+hat_corr),dot(CNN,(hat_psi_star+hat_corr))) )\
   -(0.5*delta_h*H0*H0/(h_n1**2)) * ( FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0 )* \
                                    ( (psi_1_n0+corr_nz)**2*A11 + 2.0*dot((hat_psi_star+hat_corr),AN1)*(psi_1_n0+corr_nz) \
                                                                    + dot((hat_psi_star+hat_corr),dot(ANN,(hat_psi_star+hat_corr))) ) \
                       + g*H0*delta_h*(h_n1 - H) ) )*dx
...
def WF_hat_psi_SE_SV(dim, H0, n_z, dt, b, FWF, delta_hat_psi, hat_psi, h_n0, psi_1_n0, 
    ...
    if dim=="3D":
        ...
        a_hat_psi =  h_n0*elem_mult(delta_hat_psi.dx(0),dot(MNN,hat_psi.dx(0)))\
                    ...
                    + (H0*H0/h_n0) * (  FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0 ) * elem_mult(delta_hat_psi,dot(ANN,hat_psi))\
                    +(2.0*H0/h_n0) * (b.dx(0)*h_n0.dx(0)+ b.dx(1)*h_n0.dx(1)) * elem_mult(delta_hat_psi,dot(CNN,hat_psi))\
                    ...
                    -H0*b.dx(0)* ( elem_mult(delta_hat_psi, dot(BNN.T,hat_psi.dx(0))) + elem_mult(delta_hat_psi.dx(0),dot(BNN,hat_psi)) )\
                    -H0*b.dx(1)* ( elem_mult(delta_hat_psi, dot(BNN.T,hat_psi.dx(1))) + elem_mult(delta_hat_psi.dx(1),dot(BNN,hat_psi)) )\
        ...
        L_hat_psi = -( h_n0*elem_mult(delta_hat_psi.dx(0), MN1*(psi_1_n0.dx(0)+U0_nz))\
                      ...
                      + (H0*H0/h_n0) * (  FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0 ) * elem_mult(delta_hat_psi,AN1*(psi_1_n0+corr_nz))\
                      +(2.0*H0/h_n0) * (b.dx(0)*h_n0.dx(0)+ b.dx(1)*h_n0.dx(1)) * elem_mult(delta_hat_psi,CN1*(psi_1_n0+corr_nz))\
                      ...
                      -H0*b.dx(0)* ( elem_mult(delta_hat_psi, B1N*(psi_1_n0.dx(0)+U0_nz))         + elem_mult(delta_hat_psi.dx(0),BN1*(psi_1_n0+corr_nz)))\
                      -H0*b.dx(1)* ( elem_mult(delta_hat_psi, B1N*(psi_1_n0.dx(1)+corr_nz.dx(1))) + elem_mult(delta_hat_psi.dx(1),BN1*(psi_1_n0+corr_nz))) )\
                    -( h_n0*elem_mult(delta_hat_psi.dx(0),dot(MNN,hat_U0))\
                      ...
                      + (H0*H0/h_n0) * (  FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0 ) * elem_mult(delta_hat_psi,dot(ANN,hat_corr))\
                      +(2.0*H0/h_n0) * (b.dx(0)*h_n0.dx(0)+ b.dx(1)*h_n0.dx(1)) * elem_mult(delta_hat_psi,dot(CNN,hat_corr))\
                      ...
                      -H0*b.dx(0)* ( elem_mult(delta_hat_psi, dot(BNN.T,hat_U0))         + elem_mult(delta_hat_psi.dx(0),dot(BNN,hat_corr)))\
                      -H0*b.dx(1)* ( elem_mult(delta_hat_psi, dot(BNN.T,hat_corr.dx(1))) + elem_mult(delta_hat_psi.dx(1),dot(BNN,hat_corr))) )
...
def WF_psi_half_SV(dim, n_z, g, H, H0, dt, b, FWF, delta_h_sv, delta_hat_psi_sv, psi_1_n0, psi_1_half, hat_psi_star, h_n0, 
                   C11, CN1, CNN, B11, B1N, BN1, BNN, M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN,
                   U0_nz, hat_U0, corr_nz, hat_corr):
                    ...
                    +( 0.5*delta_h_sv*( ((psi_1_half.dx(0)+U0_nz)**2)*M11 \
                      ...
                      +(1.0/h_n0) * ( delta_h_sv.dx(0)*h_n0.dx(0) + delta_h_sv.dx(1)*h_n0.dx(1) -(delta_h_sv/(2.0*h_n0))*(h_n0.dx(0)**2 + h_n0.dx(1)**2) ) *\
                                    ( (psi_1_half+corr_nz)**2*S11 + 2.0*dot((hat_psi_star+hat_corr),SN1)*(psi_1_half+corr_nz) \
                                       + dot((hat_psi_star+hat_corr),dot(SNN,(hat_psi_star+hat_corr))) )\
-(0.5*delta_h_sv*H0*H0/(h_n0**2)) * ( FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0 ) *\
                                    ( (psi_1_half+corr_nz)**2*A11 + 2.0*dot((hat_psi_star+hat_corr),AN1)*(psi_1_half+corr_nz) \
                                       + dot((hat_psi_star+hat_corr),dot(ANN,(hat_psi_star+hat_corr))) )\
                      + (H0/h_n0) * ( delta_h_sv.dx(0)*b.dx(0) + delta_h_sv.dx(1)*b.dx(1) -(delta_h_sv/h_n0)*(h_n0.dx(0)*b.dx(0) + h_n0.dx(1)*b.dx(1)) ) *\
                                    ( (psi_1_half+corr_nz)**2*C11 + 2.0*dot((hat_psi_star+hat_corr),CN1)*(psi_1_half+corr_nz) \
                                       + dot((hat_psi_star+hat_corr),dot(CNN,(hat_psi_star+hat_corr))) )\
                      + g*H0*delta_h_sv*(h_n0 - H) ))*dx
        ...
        WF_hat_psi_star=   h_n0 * elem_mult(delta_hat_psi_sv.dx(0),(MN1*(psi_1_half.dx(0)+U0_nz)        +dot(MNN,(hat_psi_star.dx(0)+hat_U0))))\
                   ...
                   - H0*b.dx(0) * ( elem_mult(delta_hat_psi_sv,(B1N*(psi_1_half.dx(0)+U0_nz)  +dot(BNN.T,(hat_psi_star.dx(0)+hat_U0))))\
                                   +elem_mult(delta_hat_psi_sv.dx(0),(BN1*(psi_1_half+corr_nz)+dot(BNN,(hat_psi_star+hat_corr)))))\
                   - H0*b.dx(1) * ( elem_mult(delta_hat_psi_sv,(B1N*(psi_1_half.dx(1)+corr_nz.dx(1))+dot(BNN.T,(hat_psi_star.dx(1)+hat_corr.dx(1)))))\
                                   +elem_mult(delta_hat_psi_sv.dx(1),(BN1*(psi_1_half+corr_nz)+dot(BNN,(hat_psi_star+hat_corr)))))\
            + (1.0/h_n0) * elem_mult(delta_hat_psi_sv,(SN1*(psi_1_half+corr_nz) + dot(SNN,(hat_psi_star+hat_corr)))) * (h_n0.dx(0)**2 + h_n0.dx(1)**2)\
          + (H0*H0/h_n0) * elem_mult(delta_hat_psi_sv,(AN1*(psi_1_half+corr_nz) + dot(ANN,(hat_psi_star+hat_corr)))) * (FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0)\
         + (2.0*H0/h_n0) * elem_mult(delta_hat_psi_sv,(CN1*(psi_1_half+corr_nz) + dot(CNN,(hat_psi_star+hat_corr)))) * (h_n0.dx(0)*b.dx(0) + h_n0.dx(1)*b.dx(1))
...
def WF_h_SV(dim, n_z, H0, g, dt, b, FWF, delta_psi, delta_hat_star, h_n0, h_n1, psi_1_half, hat_psi_star, hat_psi_aux, 
            C11, CN1, CNN, B11, B1N, BN1, BNN, M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN, 
            U0_nz, hat_U0, corr_nz, hat_corr):
    ...
    if dim == "3D":
        ...
        WF_h =( (H0*delta_psi*(h_n1-h_n0))/(0.5*dt) \
                -( h_n0*(        (psi_1_half.dx(0)+U0_nz)*M11         + dot((hat_psi_star.dx(0)+hat_U0),MN1))*delta_psi.dx(0)\
                  ...
                  - H0*b.dx(0)*( delta_psi.dx(0)*(B11*(psi_1_half+corr_nz) + dot(B1N,(hat_psi_star+hat_corr))) \
                                +delta_psi*((psi_1_half.dx(0)+U0_nz)*B11 + dot((hat_psi_star.dx(0)+hat_U0),BN1)))\
                  - H0*b.dx(1)*( delta_psi.dx(1)*(B11*(psi_1_half+corr_nz) + dot(B1N,(hat_psi_star+hat_corr)))\
                                +delta_psi*((psi_1_half.dx(1)+corr_nz.dx(1))*B11 + dot((hat_psi_star.dx(1)+hat_corr.dx(1)),BN1)))\
                  +(delta_psi/h_n0) * (  ((psi_1_half+corr_nz)*S11 + dot((hat_psi_star+hat_corr),SN1)) * (h_n0.dx(0)**2 + h_n0.dx(1)**2)\
                                 + H0*H0*((psi_1_half+corr_nz)*A11 + dot((hat_psi_star+hat_corr),AN1)) * (FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0)\
                                + 2.0*H0*((psi_1_half+corr_nz)*C11 + dot((hat_psi_star+hat_corr),CN1)) * (h_n0.dx(0)*b.dx(0) + h_n0.dx(1)*b.dx(1)) ) )\
                -( h_n1*(        (psi_1_half.dx(0)+U0_nz)*M11         + dot((hat_psi_aux.dx(0)+hat_U0),MN1))*delta_psi.dx(0)\
                  ...
                  - H0*b.dx(0)*( delta_psi.dx(0)*(B11*(psi_1_half+corr_nz) + dot(B1N,(hat_psi_aux+hat_corr))) \
                                +delta_psi*((psi_1_half.dx(0)+U0_nz)*B11 + dot((hat_psi_aux.dx(0)+hat_U0),BN1)))\
                  - H0*b.dx(1)*( delta_psi.dx(1)*(B11*(psi_1_half+corr_nz) + dot(B1N,(hat_psi_aux+hat_corr)))\
                                +delta_psi*((psi_1_half.dx(1)+corr_nz.dx(1))*B11 + dot((hat_psi_aux.dx(1)+hat_corr.dx(1)),BN1)))\
                  +(delta_psi/h_n1) * (  ((psi_1_half+corr_nz)*S11 + dot((hat_psi_aux+hat_corr),SN1)) * (h_n1.dx(0)**2 + h_n1.dx(1)**2)\
                                 + H0*H0*((psi_1_half+corr_nz)*A11 + dot((hat_psi_aux+hat_corr),AN1)) * (FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0)\
                                + 2.0*H0*((psi_1_half+corr_nz)*C11 + dot((hat_psi_aux+hat_corr),CN1)) * (h_n1.dx(0)*b.dx(0) + h_n1.dx(1)*b.dx(1)) ) ) )*dx
        ...
        WF_hat_psi_aux=   h_n1 * elem_mult(delta_hat_star.dx(0),(MN1*(psi_1_half.dx(0)+U0_nz)        +dot(MNN,(hat_psi_aux.dx(0)+hat_U0))))\
                        ...
                        - H0*b.dx(0) * ( elem_mult(delta_hat_star,((psi_1_half.dx(0)+U0_nz)*B1N+dot(BNN.T,(hat_psi_aux.dx(0)+hat_U0))))\
                                        +elem_mult(delta_hat_star.dx(0),(BN1*(psi_1_half+corr_nz)+dot(BNN,(hat_psi_aux+hat_corr)))) )\
                        - H0*b.dx(1) * ( elem_mult(delta_hat_star,((psi_1_half.dx(1)+corr_nz.dx(1))*B1N+dot(BNN.T,(hat_psi_aux.dx(1)+hat_corr.dx(1)))))\
                                        +elem_mult(delta_hat_star.dx(1),(BN1*(psi_1_half+corr_nz)+dot(BNN,(hat_psi_aux+hat_corr)))) )\
           + (1.0/h_n1) * elem_mult(delta_hat_star,(SN1*(psi_1_half+corr_nz)+dot(SNN,(hat_psi_aux+hat_corr)))) * (h_n1.dx(0)**2 + h_n1.dx(1)**2)\
         + (H0*H0/h_n1) * elem_mult(delta_hat_star,(AN1*(psi_1_half+corr_nz)+dot(ANN,(hat_psi_aux+hat_corr)))) * (FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0)\
        + (2.0*H0/h_n1) * elem_mult(delta_hat_star,(CN1*(psi_1_half+corr_nz)+dot(CNN,(hat_psi_aux+hat_corr)))) * (h_n1.dx(0)*b.dx(0) + h_n1.dx(1)*b.dx(1))
...
def WF_psi_n1_SV(dim, H0, H, g, dt, b, FWF, delta_h, psi_1_half, psi_1, hat_psi_aux, h_n1, 
                 C11, CN1, CNN, M11, MN1, MNN, D11, D1N, DN1, DNN, S11, SN1, SNN, A11, AN1, ANN,
                 U0_nz, hat_U0, corr_nz, hat_corr):
    ...
    if dim=="3D":
        ...
        L_psi_1 = ( H0*delta_h*psi_1_half\
         ...
         -(0.5*delta_h*H0*H0/(h_n1**2)) * ( FWF*(b.dx(0)**2 + b.dx(1)**2) + 1.0)*\
                                          ( (psi_1_half+corr_nz)**2*A11 + 2.0*dot((hat_psi_aux+hat_corr),AN1)*(psi_1_half+corr_nz) \
                                                                            + dot((hat_psi_aux+hat_corr),dot(ANN,(hat_psi_aux+hat_corr))) )\
                             +(H0/h_n1) * ( delta_h.dx(0)*b.dx(0) + delta_h.dx(1)*b.dx(1) - (delta_h/h_n1)*(h_n1.dx(0)*b.dx(0) + h_n1.dx(1)*b.dx(1)) )*\
                                          ( (psi_1_half+corr_nz)**2*C11 + 2.0*dot((hat_psi_aux+hat_corr),CN1)*(psi_1_half+corr_nz) \
                                                                            + dot((hat_psi_aux+hat_corr),dot(CNN,(hat_psi_aux+hat_corr))) )\
                             +g*H0*delta_h*(h_n1 - H) ) )*dx
